...
export class PugController implements IServerFull, IServerUnfull, IPug {
    ...
    public notifySubscribers(msg: string): Promise<Message> {
        const role = this.context.guild.roles.cache.find(role => role.name === 'pug-notification');
    ...
    private displayReadyStatus(): Promise<Message> {
        ...
        for (let i = 0; i < this.maxPlayers; i++) {
            ...
            if (currPlayer == null)
                ...
                let readyString: string = currentTime > currPlayer.readyUntilTime ? "<unready>" : "<ready>";
    ...
    public displayQueue(): Promise<Message> {
        ...
        for (let i = 0; i < this.pugQueue.getLength(); i++) {
            output = output.concat(this.pugQueue.getPlayer(i).player.discordMember.displayName).concat("), (");
        }
        output = output.slice(0, -3);
    ...
    public startPug(context: Message): Promise<Message> {
        ...
        this.clearInputRoles()
            ...
            .then((openServers) => new Promise((resolve, reject) => {
                if (openServers.length == 0) 
                    reject()
                else {
                    this.pug = new Pug(openServers[0], this.maxPlayers, this.getRandomMap(), this.readyDuration, this, this, this);
                    let iterations: number = this.pugQueue.getLength() > this.maxPlayers ? this.maxPlayers : this.pugQueue.getLength();
            ...
            .catch(() => {
                return this.context.channel.send("Cannot start a game, no open servers to use.");
            });
    }
    ...
    public addPlayer(member: GuildMember): Promise<Message> {
        if (this.pug == null)
            return this.context.channel.send("There is no pug in progress.");
        if (this.pug.isFull())
            return this.context.channel.send("The pug is currently full, wait until a new one has been created.");
    ...
    public addPlayerID(id: string): Promise<Message> {
        return new Promise((resolve, reject) => {
            this.context.guild.members.fetch()
            .then((members) => {
                ...
                if (member != null) 
                    resolve(this.addPlayer(member));
                else
                    resolve(this.context.channel.send(`No player found with the ID: ${id}`))
            })
        })
    }
    ...
    public removePlayer(id: string): Promise<Message> {
        if (this.pug == null)
            return this.context.channel.send("There is no pug in progress.");
        if (this.pug.removePlayer(id))
            return this.displayReadyStatus();
        else
            return this.context.channel.send("You are not added to the pug.") 
    }
    ...
    public vacateServer(serverID: number): Promise<Message> {
        ...
        if (currServer != null) {
            let rcon = new RconConnection(currServer.address, currServer.port, this.rconPassword);
            rcon.sendCommand("kickall")
                .then(() => {
                    return this.context.channel.send("All players kicked from the server");
                })
                .catch(() => {
                    return this.context.channel.send("Unable to kick players from the server");
                })    
        }
    ...
    public readyPlayer(id: string, duration: number) {
        let validDuration = duration > this.maxReadyDuration ? this.maxReadyDuration : duration;
        if (validDuration < this.minReadyDuration) validDuration = this.minReadyDuration;
        ...
        this.context.channel.send(`${temp.discordMember.displayName} will be ready for the next ${validDuration} minutes`);
    }

    public queuePlayer(member: GuildMember): Promise<Message> {
        //If a pug is on, just add as normal
        if (this.pug != null)
            return this.addPlayer(member)
        
        if (this.pugQueue.queuePlayer(new Player(member))) 
            return this.displayQueue();
        else 
            return this.context.channel.send("Already added to the queue.");
    }

    public dequeuePlayer(id: string): Promise<Message> {
        if (this.pugQueue.removePlayerID(id))
            return this.displayQueue();
        else 
            return this.context.channel.send("You were not added to the queue.");
    }
    ...
    private clearInputRoles(): Promise<void> {
        return new Promise((resolve, reject) => {
            const role = this.context.guild.roles.cache.find(role => role.name === 'in-pug');
            const list = this.context.guild.members.fetch()
                .then(members => {
                    members.forEach((m,s) => {
                        m.roles.remove(role)
                    })
    ...
    private sendConnectionDetails(server: TF2Server) {
        ...
        rcon.sendCommand(`changelevel ${this.pug.currentMap}`)
            .then(() => {
    ...
    onServerFull(server: TF2Server) {
        ...
        if (unreadyPlayers.length == 0) {
            this.context.channel.send(`<@&${role.id}> Your pickup game is full, everyone is ready. Sending connection details`)
                .then(() => this.sendConnectionDetails(server));
        } else {
            let reactMessage: Message;
            this.context.channel.send(`<@&${role.id}> Your pickup game is full, react below to ready-up`)
                .then((message) => new Promise((resolve, reject) => {
                    ...
                    unreadyPlayers.forEach((p) => {
                        unreadyOutput = unreadyOutput.concat(p.discordMember.displayName).concat(",");
                    })
                    unreadyOutput.slice(0,-1);
                    resolve(unreadyOutput)
                }))
                .then((unreadyOutput) => this.context.channel.send(unreadyOutput))
                .then((message) => {  //message - the message sent to discord displaying the unready players, will be used to update dynamically
                    readyMessage = message;  
                    
                    this.checkReadyInterval = setInterval(() => {
                        //get list of all who have reacted
                        reactMessage.reactions.resolve("👍").users.fetch()
                            .then((users) => new Promise<void> ((resolve, reject) => {
                                users.forEach((u,k,m) => {
                    ...
                    this.readyTimeout = setTimeout(() => {
                        ...
                        this.context.channel.send(`The following players are not ready and have been removed from the pug: ${output}`);
    ...
    onServerUnfull() {
        this.context.channel.send("Pickup game no longer full, cancelling ready check.")
        clearTimeout(this.readyTimeout);
        clearTimeout(this.checkReadyInterval);
    }
