...
export class Pug {
    ...
    constructor(server: TF2Server, maxPlayers: number, map: string, readyDuration: number, serverFull: IServerFull, serverUnfull: IServerUnfull, pugListener: IPug) {
        this.server = server;
    ...
    public addPlayer(player: Player): boolean {   
        ...
        this.addedPlayers.forEach(m => {
            if (m.discordMember.id === player.discordMember.id)
                added = true;
        });
    ...
    public removePlayerRegex(name: string): number {    //0 - successfully kicked, 1 - no player found, 2 - multiple players match the name
        let contains: Player[] = [];
        this.addedPlayers.forEach(p => {
    ...
    public getUnreadyPlayers(): Player[] {
        ...
        this.addedPlayers.forEach(player => {
            if (currentTime > player.readyUntilTime)
